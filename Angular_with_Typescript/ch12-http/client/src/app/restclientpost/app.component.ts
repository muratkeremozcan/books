...
// [3] http.post: this is an example of POST request from the UI (via form submit) and the server responding
// high level:
...
export class AppComponent {
  ...
  addProduct(payload) {
    ...
    this.httpClient.post<string>('/api/product', payload).subscribe( // (3.2.4) handle the response in a subscribe
      ...
      // err.error.message  may contain more info. To trigger an error, turn off the server
...
// to run:
// UI (client folder):  ng serve restclientpost -o --proxy-config proxy-conf.json
// API (server folder): node build/rest-server-angular-post