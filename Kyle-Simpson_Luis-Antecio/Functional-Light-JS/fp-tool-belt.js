...
export const partialThis = (fn, ...presentArgs) =>
  function partiallyApplied(...laterArgs) {
    // intentional function to allow this binding
 ...
 */
export const partialProps = (fn, presentArgsObj) => (laterArgsObj) =>
  fn(Object.assign({}, presentArgsObj, laterArgsObj));
...
export const compose =
  ...
  (result) => {
    ...
    while (list.length > 0) {
      // take the last function off the end of the list and execute it with result argument
...
export const pipe =
  ...
  (result) => {
    let list = [...fns]; // copy the array of functions
    while (list.length > 0) {
      // take the first function off the beginning of the list and execute
...
export const mergeReducer = (merged, v, idx) => (
  merged.splice(idx * 2, 0, v), merged
);

/** FP style filter. Array is received last. */
export const filter = curry((predicateFn, arr) => arr.filter(predicateFn));
 ...
 * `map( v => [v, v + 1], x)` //? [[3, 4]]
*/
export const map = curry((mapperFn, arr) => arr.map(mapperFn));
 ...
 * `map( v => [v, v + 1], x)` //? [3, 4]
*/
export const flatMap = curry((mapperFn, arr) =>
 ...
 */
export const unboundMethod = (methodName, argCount = 2) =>
  curry((...args) => {
    let obj = args.pop();
 ...
 */
export const guard = (fn) => (arg) => arg != null ? fn(arg) : arg;
...
export function Just(val) {
  // notice that val is never changed
...
export const alt = curry((func1, func2, val) =>
  func1 ? func1(val) : func2(val)
);

/** process a single resource (val) in two different ways (f1, f2) and then combine the results */
export const fork = (join, func1, func2) => (val) =>
