...
app.use( // to load Bootstrap's css, use express.static to register the file at the right url
  '/css/bootstrap.css',
...
app.get('/articles', (req, res, next) => {
  Article.all((err, articles) => { // all method of Article class with callback, , utilizing sqlite3.all
    ...
    res.format({ // res.format method of Express allows the app to respond with the right format
      html: () => { // if html , respond with this functino
        res.render('articles.ejs', { articles });
      },
      json: () => { // if json, respond with default
        res.send(articles);
      }
    });
  });
