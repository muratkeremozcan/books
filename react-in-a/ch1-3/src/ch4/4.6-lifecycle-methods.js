...
// Stateless functional components don’t have lifecycle methods available to them.
// You also can’t use this.setState inside them because they don’t have backing instances; (except when using hooks..)
...
class ChildComponent extends React.Component {
  ...
  componentDidMount() {
    // render into the DOM first, then this - mounting.2
  ...
  componentDidUpdate(previousProps, previousState) {
    // render into the DOM, then this - updating.4
  ...
  oops() {
    // error handling.2 : toggle state to throw an error
    this.setState(() => ({ oops: true }));
  }
  render() {
    if (this.state.oops) {
      // error handling.3 : throw error in th render method
...
class ParentComponent extends React.Component {
  ...
  componentDidCatch(err, errorInfo) {
    // error handling.4 : add a componentDidCatch to the child and use it to update component state
