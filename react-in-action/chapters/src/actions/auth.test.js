jest.mock("../../src/backend/auth"); // mock the external dependencies of auth module
...
describe("login actions", () => {
  ...
  test("loginSuccess", () => {
    ...
    const token = '123abc';
    const expected = { type: types.auth.LOGIN_SUCCESS, user: mockUser, token};
    const actual = loginSuccess(mockUser, token);
  ...
  test("logout (success)", async () => {
    AUTH.logUserOut = jest.fn(() => Promise.resolve()); // mock successful logout
  ...
  test("logout with error", async () => {
    const mockError = new Error("oops"); // mock error in logout
    ...
    const expectedActions = [
      { type: types.app.ERROR, error: mockError, info: undefined }
    ];
    expect(actions).toEqual(expectedActions);
  });
